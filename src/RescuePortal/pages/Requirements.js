...
const Requirements = () => {
  ...
  return (
    <div>
      <Navbar />
      <div className="grid grid-cols-12 justify-center items-center px-12 py-12 bg-gradient-to-t from-[#BAAC9A] to-gray-300">
        <div className="grid grid-cols-12 gap-4 col-start-2 col-span-10 justify-center items-center rounded-sm">
          {loc.map((element) => (
            <Link to={element.src} className="flex flex-col col-span-3 px-4 py-4 shadow-md h-[350px] shadow-[#62553B] rounded-md bg-gradient-to-t from-[#2F2D29] to-[#62553B] transition ease-in-out delay-50 hover:-translate-y-2 duration-500">
              <div className="flex relative overflow-visible">
                <img
                  src={element.src}
                  className="inset-0 w-64 object-fill object-right z-10 rounded-md"
                />
                <div className="bg-gray-300 absolute inset-0 z-20 opacity-30"></div>
                <Link className="flex absolute z-30" to="">
                  <i className="ri-map-pin-2-fill text-2xl my-2 mx-2 rounded-md"></i>
                </Link>
              </div>
              <div className="flex flex-col">
                <span className="text-xl text-gray-100 my-2">{element.req}</span>
                <span className="text-sm text-gray-100">{element.sub}</span>
              </div>
            </Link>
          ))}
        </div>
